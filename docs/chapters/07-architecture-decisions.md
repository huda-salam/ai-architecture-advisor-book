...
The advisor's job is not simply to identify a technically possible solution. It is to make the decision structure explicit: what is being decided, why, what alternatives exist, what trade-offs are accepted, and under what conditions the decision should be revisited.
...
## 7.4 Architecture Decision Record (ADR)
...
## 7.5 Decision Criteria Must Come From the Problem
...
A deliberate shortcut may be rational when:

- the requirement is uncertain;
- the solution is experimental;
- time-to-value matters;
- the decision is reversible;
- the cost of postponing complexity is acceptable.

It becomes dangerous when debt is:

- invisible;
- accumulating;
- difficult to quantify;
- blocking future changes;
- creating security or reliability exposure.
...
Examples of potentially difficult commitments include:

- proprietary data formats;
- deeply coupled vendor APIs;
- large specialized infrastructure investments;
- architecture that embeds one model's behavior throughout the application;
- workflows that cannot operate without one provider.

The more difficult a decision is to reverse, the stronger the evidence should generally be before committing.

## 7.11 Decision Confidence
...
For example, instead of immediately building a GPU cluster:

> benchmark expected workloads → measure utilization → estimate TCO → then decide whether self-hosting is justified.

## 7.13 Second Opinion and Dissent Record
...
This preserves independence without converting technical disagreement into organizational conflict.

## 7.14 Decision Review
...
## 7.15 Architecture Decision Quality
...
2. Were the important constraints identified?
3. Were material alternatives considered?
4. Were assumptions explicit?
5. Was evidence appropriate?
6. Were trade-offs understood?
7. Were risks consciously accepted?
8. Was the decision proportionate to the stakes?
9. Was reversibility considered?
10. Can the reasoning be reconstructed later?
...
> **Frame → Constrain → Identify → Compare → Test → Recommend → Record → Review**

**Frame** — Define the actual problem.
...
> **Make important architecture decisions explicit, evidence-based, proportionate to the stakes, and reversible where practical. Document not only what was chosen, but why—and what would cause us to change it.**
